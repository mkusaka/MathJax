  ...
  if (!(window.MathJax && MathJax.Hub)) {
    ...
    MathJax.HTML = {
      ...
      Element: function(type, def, contents) {
        ...
        if (def) {
          ...
          for (id in def) {
            if (id === "role" || id.substr(0, 5) === "aria-")
              obj.setAttribute(id, def[id]);
          }
        }
        if (contents) {
          if (!MathJax.Object.isArray(contents)) {
            contents = [contents];
          }
          for (var i = 0, m = contents.length; i < m; i++) {
            if (MathJax.Object.isArray(contents[i])) {
              ...
              // IE throws an error if script is added as a text node
    ...
    MathJax.Localization = {
      ...
      _: function(id, phrase) {
        if (MathJax.Object.isArray(phrase)) {
              ...
              if (match) {
                if (match[1] === "plural") {
                  ...
                  if (typeof n === "undefined") {
                    ...
                    var plurals = match[3]
                      .replace(/(^|[^%])(%%)*%\|/g, "$1$2%\uEFEF")
                      .split(/\|/); // the parts (replacing %| with a special character)
      ...
      concatString: function(result, string, args, domain) {
        if (string != "") {
          ...
          if (!MathJax.Object.isArray(string)) {
            string = [string];
          }
          ...
          if (!localeData.isLoaded) {
            ...
            if (load) {
              return MathJax.Callback.Queue(
                load,
                ["loadDomain", this, domain] // call again to load domain
              ).Push(callback || {});
            }
          }
          if (localeData.domains && domain in localeData.domains) {
      ...
      Init: function(styles) {
        ...
        if (!this.div) {
          ...
          if (this.msieFixedPositionBug && window.attachEvent) {
            ...
            frame.style.border = frame.style.margin = frame.style.padding =
              "0px";
    ...
    MathJax.Hub = {
      ...
      isMathJaxNode: function(element) {
        return (
          !!element &&
          (element.isMathJax || (element.className || "") === "MathJax_MathML")
        );
      },
        ...
        if (!state.jax[jax.outputJax]) {
          if (state.jaxIDs.length === 0) {
            ...
            if (state.jaxIDs.length === 1) {
              // get the script so far for the existing jax
      ...
      formatError: function(script, err) {
        ...
        if (err.sourceURL || err.fileName)
          message +=
            "\n" +
            LOCALIZE("ErrorFile", "file: %1", err.sourceURL || err.fileName);
        if (err.line || err.lineNumber)
          message +=
            "\n" +
        ...
        var errorText = LOCALIZE(
          errorSettings.messageId,
          errorSettings.message
        );
        var error = MathJax.HTML.Element(
          "span",
          {
      ...
      elementCallback: function(element, callback) {
        if (
          callback == null &&
          (MathJax.Object.isArray(element) || typeof element === "function")
        ) {
          try {
        ...
        if (!MathJax.Object.isArray(element)) {
          element = [element];
        }
    ...
    MathJax.Hub.Startup = {
      ...
      Jax: function() {
        ...
        for (var i = 0, m = config.jax.length, k = 0; i < m; i++) {
          ...
          if (
            config.jax[i].substr(0, 7) === "output/" &&
            jax.order[name] == null
          ) {
            jax.order[name] = k;
            k++;
      ...
      loadArray: function(files, dir, name, synchronous) {
        if (files) {
          if (!MathJax.Object.isArray(files)) {
            files = [files];
          }
          if (files.length) {
            var queue = MathJax.Callback.Queue(),
      ...
      BASE.InputJax = JAX.Subclass(
        {
          ...
          Process: function(script, state) {
            ...
            for (var i = 0, m = jax.length; i < m; i++) {
              ...
              if (!this.require) {
                this.require = [];
              } else if (!BASE.Object.isArray(this.require)) {
    ...
    (function(BASENAME) {
        ...
        if (BROWSERS.hasOwnProperty(browser)) {
          if (BROWSERS[browser] && browser.substr(0, 2) === "is") {
            ...
            var VERSION = new RegExp(
              ...
              ")" +
              (browser == "MSIE" ? " " : "/") +
              "((?:\\d+\\.)*\\d+)|" + // for one of the main browsers
                "(?:^|\\(| )([a-z][-a-z0-9._: ]+|(?:Apple)?WebKit)/((?:\\d+\\.)+\\d+)"
            ); // for unrecognized browser
      ...
      try {
        HUB.Browser.Select({
          ...
          Firefox: function(browser) {
            if (
              (browser.version === "0.0" || AGENT.match(/Firefox/) == null) &&
              ...
              if (rv) {
                ...
                if (date >= "20111220") {
                  browser.version = "9.0";
                } else if (date >= "20111120") {
                  browser.version = "8.0";
                } else if (date >= "20110927") {
                  browser.version = "7.0";
                } else if (date >= "20110816") {
                  browser.version = "6.0";
                } else if (date >= "20110621") {
                  browser.version = "5.0";
                } else if (date >= "20110320") {
                  browser.version = "4.0";
                } else if (date >= "20100121") {
                  browser.version = "3.6";
                } else if (date >= "20090630") {
                  browser.version = "3.5";
                } else if (date >= "20080617") {
                  browser.version = "3.0";
                } else if (date >= "20061024") {
                  browser.version = "2.0";
                }
              }
            }
          ...
          Chrome: function(browser) {
            browser.noContextMenu = browser.isMobile = !!navigator.userAgent.match(
              / Mobile[ \/]/
            );
          },
      ...
      if (
        BASE.AuthorConfig &&
        typeof BASE.AuthorConfig.AuthorInit === "function"
      ) {
      ...
      HUB.queue.Push(
        ...
        function() {
          ...
          var queue = BASE.Callback.Queue(STARTUP.Jax(), STARTUP.Extensions());
          return queue.Push({});
        },
        ["Menu", STARTUP],
        STARTUP.onLoad(),
