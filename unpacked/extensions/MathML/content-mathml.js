...
MathJax.Extension["MathML/content-mathml"] = (function(HUB) {
  ...
  var CToP = {
    ...
    transformElement: function(element) {
      if (element.nodeName.indexOf(":") >= 0)
        element = CToP.cloneNode(element, true); // removes namespaces
    ...
    transforms: {
      ...
      token: function(name) {
        return function(parentNode, contentMMLNode) {
          if (
            contentMMLNode.childNodes.length === 1 &&
            contentMMLNode.childNodes[0].nodeType === 3
          ) {
            CToP.appendToken(
              parentNode,
              name,
              CToP.getTextContent(contentMMLNode)
            );
          } else {
            var mrow = CToP.createElement("mrow");
            for (var j = 0, l = contentMMLNode.childNodes.length; j < l; j++) {
      ...
      binary: function(name, tokenPrecedence) {
        return function(
          ...
          var needsBrackets =
            ...
            (tokenPrecedence == precedence && name === "-");
          if (needsBrackets) {
            CToP.appendToken(mrow, "mo", "(");
          }
          if (args.length > 1) {
      ...
      iteration: function(name, limitSymbol) {
        return function(
          ...
          var i,
            ...
            num_args;
          for (
            i = 0, num_qualifiers = qualifiers.length;
            i < num_qualifiers;
            i++
          ) {
            if (
              qualifiers[i].nodeName === "lowlimit" ||
              ...
              children = CToP.getChildren(qualifiers[i]);
              if (
                qualifiers[i].nodeName === "interval" &&
                children.length === 2
              ) {
                for (j = 0, num_bvars = bvars.length; j < num_bvars; j++) {
                  bvar = bvars[j];
                ...
                CToP.applyTransform(
                  mrow1,
                  CToP.getChildren(qualifiers[i])[0],
                  0
                );
              }
            }
          ...
          for (
            ...
            if (
              qualifiers[i].nodeName === "uplimit" ||
              qualifiers[i].nodeName === "interval"
            ) {
              children = CToP.getChildren(qualifiers[i]);
              for (
                j = 0, num_children = children.length;
                j < num_children;
                j++
              ) {
  ...
  CToP.tokens = {
    ...
    fn: function(parentNode, contentMMLNode, precedence) {
      CToP.applyTransform(
        parentNode,
        CToP.getChildren(contentMMLNode)[0],
        precedence
      );
    },
    ...
    interval: function(parentNode, contentMMLNode, precedence) {
      ...
      parentNode.appendChild(
        CToP.createmfenced(CToP.getChildren(contentMMLNode), open, close)
      );
    },
    ...
    apply: function(parentNode, contentMMLNode, precedence) {
      ...
      if (firstArg) {
        ...
        name =
          name === "csymbol"
            ? CToP.getTextContent(firstArg).toLowerCase()
            : name;
        if (CToP.applyTokens[name]) {
          CToP.applyTokens[name](
            parentNode,
            contentMMLNode,
            firstArg,
            args,
        ...
        switch (type) {
          ...
          case "hexdouble":
            CToP.appendToken(
              parentNode,
              "mn",
              "0x" + CToP.getTextContent(contentMMLNode)
            );
            break;
          default:
            ...
            for (var j = 0, l = contentMMLNode.childNodes.length; j < l; j++) {
              if (contentMMLNode.childNodes[j].nodeType === 3) {
                ...
                mrow.appendChild(
                  CToP.cloneNode(contentMMLNode.childNodes[j], true)
                );
              }
            }
            apply.appendChild(mrow);
      ...
      if (bvars.length) {
        ...
        if (qualifiers.length) {
          ...
          for (
            ...
            children = CToP.getChildren(qualifiers[i]);
            for (
              var j = 0, num_children = children.length;
              j < num_children;
              j++
            ) {
    ...
    semantics: function(parentNode, contentMMLNode, precedence) {
      ...
      if (children.length) {
        ...
        for (var i = 0, l = children.length; i < l; i++) {
          if (
            children[i].nodeName === "annotation-xml" &&
            children[i].getAttribute("encoding") === "MathML-Presentation"
          ) {
            z = children[i];
            break;
          }
  ...
  CToP.applyTokens = {
    ...
    not: function(
      ...
      CToP.appendToken(mrow, "mo", "\u00ac");
      var needsBrackets =
        args[0].nodeName === "apply" || args[0].nodeName === "bind";
      if (needsBrackets) {
        CToP.appendToken(mrow, "mo", "(");
      }
      CToP.applyTransform(mrow, args[0], precedence);
        ...
        if (j > 0) {
          ...
          if (CToP.settings.collapsePlusMinus) {
            if (
              arg.nodeName === "cn" &&
              !children.length &&
              (n = Number(CToP.getTextContent(arg))) < 0
            ) {
              CToP.appendToken(mrow, "mo", "\u2212");
              ...
              arg.nodeName === "apply" &&
              children.length > 2 &&
              children[0].nodeName === "times" &&
              children[1].nodeName === "cn" &&
              (n = Number(CToP.getTextContent(children[1]))) < 0
            ) {
              CToP.appendToken(mrow, "mo", "\u2212");
    ...
    root: function(
      ...
      var mr;
      if (
        firstArg.nodeName === "root" &&
        (qualifiers.length === 0 ||
          (qualifiers[0].nodeName === "degree" &&
            CToP.getTextContent(qualifiers[0]) === "2"))
      ) {
        ...
        var arg =
          firstArg.nodeName === "root" ? qualifiers[0].childNodes[0] : args[1];
    ...
    partialdiff: function(
      ...
      if (
        bvars.length === 0 &&
        args.length === 2 &&
        args[0].nodeName === "list"
      ) {
        if (args[1].nodeName === "lambda") {
          // `d^(n+m)/(dx^n dy^m) f` form, through a lambda
        ...
        // `d^(n+m)/(dx^n dy^m) f` form, with bvars
        msup = CToP.createElement("msup");
        toprow.appendChild(msup);
        CToP.appendToken(msup, "mo", "\u2202");
        ...
        if (
          qualifiers.length &&
          qualifiers[0].nodeName === "degree" &&
          CToP.getChildren(qualifiers[0]).length
        ) {
