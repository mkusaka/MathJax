...
(function(HUB, HTML, AJAX, CALLBACK, OUTPUT) {
  ...
  var CONFIG = HUB.CombineConfig("MathMenu", {
    ...
    styles: {
      "#MathJax_About": {
        ...
        filter:
          "progid:DXImageTransform.Microsoft.dropshadow(OffX=2, OffY=2, Color='gray', Positive='true')" // IE
      },
  ...
  var MENU = (MathJax.Menu = NAV.Subclass(
    {
      ...
      Post: function(event, parent, forceLTR) {
        ...
        var styles = window.getComputedStyle
          ...
          : { marginLeft: "0px" };
        var bodyRight =
          bbox.right - Math.min(0, bbox.left) + parseFloat(styles.marginLeft);
    ...
    },
    {
      ...
      Background: function(menu) {
        var div = HTML.addElement(
          ...
          [
            [
              "div",
              { style: this.BGSTYLE, menuItem: menu, onmousedown: this.Remove }
            ]
          ]
        );
      ...
      MoveHorizontal: function(event, menu, move) {
        ...
        if (len === 0) return;
        var next =
          jaxs[MENU.Mod(move(MENU.IndexOf(jaxs, MENU.CurrentNode())), len)];
  ...
  var ITEM = (MENU.ITEM = NAV.Subclass(
    {
      ...
      RemoveSubmenus: function(menu, menus) {
        menus =
          menus || document.getElementById("MathJax_MenuFrame").childNodes;
  ...
  MENU.ENTRY = MENU.ITEM.Subclass({
    ...
    MoveVertical: function(event, item, move) {
      ...
      var items = [];
      for (
        var i = 0, allItems = menuNode.menuItem.items, it;
        (it = allItems[i]);
        i++
      ) {
        if (!it.hidden) {
  ...
  MENU.ITEM.RADIO = MENU.ENTRY.Subclass({
    ...
    Attributes: function(def) {
      var checked =
        CONFIG.settings[this.variable] === this.value ? "true" : "false";
  ...
  MENU.About = function(event) {
      ...
      [
        ...
        [
          HUB.Browser +
            " v" +
            HUB.Browser.version +
            (format ? " \u2014 " + _(format.replace(/ /g, ""), format) : "")
        ]
      ]
    );
    ...
    if (MENU.prototype.msieAboutBug) {
      ...
      about.style.left =
        Math.floor(
          (document.documentElement.scrollWidth - about.offsetWidth) / 2
        ) + "px";
      about.style.top =
  ...
  MENU.ShowSource.Text = function(text, event) {
    ...
    if (MENU.isMobile) {
      ...
      setTimeout(function() {
        ...
        W = Math.max(
          140,
          Math.min(Math.floor(0.5 * screen.width), table.offsetWidth + W + 25)
        );
        H = Math.max(
          40,
          Math.min(Math.floor(0.5 * screen.height), table.offsetHeight + H + 25)
        );
        ...
        if (event && X != null) {
          x = Math.max(
            0,
            Math.min(event.screenX - Math.floor(W / 2), screen.width - W - 20)
          );
          y = Math.max(
            0,
            Math.min(event.screenY - Math.floor(H / 2), screen.height - H - 20)
          );
          w.moveTo(x, y);
        }
      }, 50);
  ...
  MENU.Scale = function() {
    ...
    var scale = prompt(
      _("ScaleMath", "Scale all mathematics (compared to surrounding text) by"),
      SCALE + "%"
    );
    if (scale) {
      if (scale.match(/^\s*\d+(\.\d*)?\s*%?\s*$/)) {
        ...
        alert(
          _("PercentScale", "The scale should be a percentage (e.g., 120%%)")
        );
      }
    }
  };
  ...
  MENU.Renderer = function() {
    ...
    if (jax[0] !== CONFIG.settings.renderer) {
      ...
      if (message) {
        ...
        MENU.saveCookie();
        if (!confirm(message)) {
          MENU.cookie.renderer = CONFIG.settings.renderer = HTML.Cookie.Get(
            "menu"
          ).renderer;
          MENU.saveCookie();
          return;
  ...
  MENU.AssistiveMML = function(item, restart) {
    ...
    MathJax.Hub.Queue([
      (CONFIG.settings.assistiveMML ? "Add" : "Remove") + "AssistiveMathML",
      AMML
    ]);
  };
  ...
  MENU.CreateLocaleMenu = function() {
    ...
    for (var i = 0, m = locales.length; i < m; i++) {
      ...
      menu.items.push(
        ITEM.RADIO([locales[i], title], "locale", { action: MENU.Locale })
      );
    }
    //
  ...
  HUB.Register.StartupHook("End Config", function() {
    ...
    MENU.menu = MENU(
      ITEM.SUBMENU(
        ["Show", "Show Math As"],
        ITEM.COMMAND(["MathMLcode", "MathML Code"], MENU.ShowSource, {
          nativeTouch: true,
          format: "MathML"
        }),
        ITEM.COMMAND(["Original", "Original Form"], MENU.ShowSource, {
          nativeTouch: true
        }),
      ...
      ITEM.SUBMENU(
        ...
        ITEM.SUBMENU(
          ...
          ITEM.CHECKBOX(
            HUB.Browser.isMac ? ["Option", "Option"] : ["Alt", "Alt"],
            "ALT"
          ),
        ...
        ITEM.SUBMENU(
          ...
          ITEM.RADIO(["GyrePagellaWeb", "Gyre Pagella (web)"], "font", {
            action: MENU.Font
          }),
          ITEM.RADIO(["GyreTermesWeb", "Gyre Termes (web)"], "font", {
            action: MENU.Font
          }),
          ITEM.RADIO(["LatinModernWeb", "Latin Modern (web)"], "font", {
            action: MENU.Font
          }),
      ...
      ITEM.SUBMENU(
        ...
        ITEM.CHECKBOX(["AssistiveMML", "Assistive MathML"], "assistiveMML", {
