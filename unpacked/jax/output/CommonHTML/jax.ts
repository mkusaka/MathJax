  ...
  CHTML.Augment({
    ...
    preTranslate: function(state) {
      ...
      for (i = 0; i < m; i++) {
        ...
        if (
          prev &&
          prev.className &&
          String(prev.className).substr(0, 9) === "mjx-chtml"
        )
        ...
        test.className +=
          " mjx-test-" + (jax.CHTML.display ? "display" : "inline");
    ...
    getUnicode: function(string) {
      ...
      if (n >= 0xd800 && n < 0xdbff) {
        n =
          ((n - 0xd800) << 10) +
          (string.text.charCodeAt(string.i) - 0xdc00) +
          0x10000;
        string.i++;
      }
      return n;
    ...
    createDelimiter: function(node, code, HW, BBOX, font) {
      ...
      if (delim.load)
        HUB.RestartAfter(
          AJAX.Require(this.fontDir + "/TeX/fontdata-" + delim.load + ".js")
        );
      ...
      return this["extendDelimiter" + delim.dir](
        node,
        hw,
        delim.stretch,
        BBOX,
    ...
    Transform: function(node, trans, origin) {
      var style = node.style;
      style.transform = style.WebkitTransform = style.MozTransform = style[
        "-ms-transform"
      ] = trans;
      if (origin)
  ...
  CHTML.BBOX = MathJax.Object.Subclass(
    {
      ...
      combine: function(cbox, x, y) {
        ...
        if (
          cbox.D &&
          (this.D == null || scale * cbox.D - y > this.D) &&
          scale * cbox.D > this.d
        )
      ...
      append: function(cbox) {
        ...
        if (
          cbox.D &&
          (this.D == null || scale * cbox.D > this.D) &&
          scale * cbox.D > this.d
        )
          this.D = scale * cbox.D;
        else if (cbox.D == null && this.D) delete this.D;
  ...
  MathJax.Hub.Register.StartupHook("mml Jax Ready", function() {
    ...
    MML.mbase.Augment(
      {
        ...
        CHTMLcoreNode: function(node) {
          if (this.inferRow && this.data[0])
            return this.data[0].CHTMLcoreNode(node.firstChild);
        ...
        CHTMLstretchChildV: function(i, H, D) {
          ...
          if (data) {
            ...
            if (
              dbox.stretch ||
              (dbox.stretch == null && data.CHTMLcanStretch("Vertical", H, D))
            ) {
              var w = dbox.w;
        ...
        CHTMLstretchChildH: function(i, W, node) {
          ...
          if (data) {
            ...
            if (
              dbox.stretch ||
              (dbox.stretch == null && data.CHTMLcanStretch("Horizontal", W))
            ) {
              var w = dbox.w;
        ...
        CHTMLcreateNode: function(node) {
          ...
          var id =
            (this.id || "MJXc-Node-" + this.CHTMLnodeID) + CHTML.idPostfix;
        ...
        CHTMLnotEmpty: function(mml) {
          while (
            mml &&
            mml.data.length < 2 &&
            (mml.type === "mrow" || mml.type === "texatom")
          )
            mml = mml.data[0];
          return !!mml;
        }
      ...
      CHTMLstretchV: function(h, d) {
        ...
        if (H !== bbox.sH) {
          ...
          if (H != values.minsize) {
            H = [
              Math.max(
                (H * CHTML.TEX.delimiterfactor) / 1000,
    ...
    MML.munderover.Augment({
      ...
      CHTMLgetBBoxes: function(bbox, nodes, values) {
        ...
        for (i = 0; i < m; i++) {
          ...
          W = Math.max(
            W,
            scale * (bbox[i].w + (bbox[i].L || 0) + (bbox[i].R || 0))
          );
      ...
      CHTMLplaceBoxes: function(base, under, over, values, boxes) {
        ...
        for (i = 0; i < m; i++) {
          ...
          boxes[i].x += { left: 0, center: (W - w) / 2, right: W - w }[
            values.align
          ];
        ...
        for (i = 0; i < m; i++) {
          if (this.data[i]) {
            ...
            if (boxes[i].x - dx) {
              var node =
                i === this.base ? base : i === this.over ? over : under;
    ...
    MML.msubsup.Augment({
      toCommonHTML: function(node, options) {
        ...
        if (bmml) {
          if (
            (bmml.type === "mrow" || bmml.type === "mstyle") &&
            bmml.data.length === 1
          )
            bmml = bmml.data[0];
        ...
        if (!sup) {
          ...
          if (!sub) {
            ...
            sup.style.paddingBottom = CHTML.Em(
              u / Sscale + v / sscale - Sbox.d - (sbox.h / sscale) * Sscale
            );
            sup.style.paddingLeft = CHTML.Em(delta / Sscale);
            sup.style.paddingRight = CHTML.Em(s / Sscale);
    ...
    MML.mfrac.Augment({
      toCommonHTML: function(node) {
        ...
        values.linethickness = Math.max(
          0,
          CHTML.thickness2em(values.linethickness || "0", BBOX.scale)
        );
        ...
        if (values.bevelled) {
          ...
          var H =
            Math.max(nscale * (nbox.h + nbox.d), dscale * (dbox.h + dbox.d)) +
            2 * delta;
          ...
          bevel.style.marginLeft = bevel.style.marginRight = CHTML.Em(
            -delta / 2
          );
    ...
    MML.mrow.Augment({
      ...
      CHTMLlineBreaks: function() {
        ...
        return (
          (LINEBREAKS.automatic && this.CHTML.w > CHTML.linebreakWidth) ||
          this.hasNewline()
        );
      },
      CHTMLstretchV: function(h, d) {
