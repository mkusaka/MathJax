...
(function(AJAX, HUB, HTMLCSS) {
  ...
  var FONTTEST = MathJax.Object.Subclass({
    ...
    testCollection: function(name) {
      var font = { testString: "() {} []" };
      font.family =
        { TeX: "MathJax_Size1", STIX: "STIXSizeOneSym" }[name] ||
    ...
    testFont: function(font) {
      ...
      if (W) {
        ...
        if (
          found === null &&
          (this.div.offsetWidth != W[3] || this.div.offsetHeight != W[4])
        ) {
          if (
            !font.noStyleChar &&
            HTMLCSS.FONTDATA &&
            HTMLCSS.FONTDATA.hasStyleChar
          ) {
    ...
    testStyleChar: function(font, size) {
      ...
      var N = Math.floor((this.div.offsetWidth - dw) / W + 0.5);
      if (N === n) {
        if (HTMLCSS.safariTextNodeBug) {
    ...
    getComparisonWidths: function(string, noStyleChar) {
      ...
      var sW = this.div.offsetWidth,
        sH = this.div.offsetHeight;
    ...
    loadWebFont: function(font) {
      HUB.Startup.signal.Post(
        "HTML-CSS Jax - Web-Font " + HTMLCSS.fontInUse + "/" + font.directory
      );
      var n = MESSAGE([
        "LoadWebFont",
        "Loading web-font %1",
      ...
      AJAX.timer.start(
        AJAX,
        [this.checkWebFont, font, callback],
        0,
        this.timeout
      );
      return done;
    ...
    loadError: function(font) {
      ...
      HUB.Startup.signal.Post(
        "HTML-CSS Jax - web font error for " +
          HTMLCSS.fontInUse +
          "/" +
          font.directory
      );
  ...
  HTMLCSS.Augment({
    ...
    Startup: function() {
      ...
      var div = this.addElement(this.hiddenDiv, "div", {
        style: { width: "5in" }
      });
    ...
    preTranslate: function(state) {
      ...
      for (i = 0; i < m; i++) {
        ...
        } else if (this.msieDisappearingBug) {
          span.style.display = "inline-block";
        }
        div.className += " MathJax_Processing";
        script.parentNode.insertBefore(div, script);
        jax.HTMLCSS.span = span;
        jax.HTMLCSS.div = div; // save for use in Translate()
        //
        //  Add the test span for determining scales and linebreak widths
        //
        test = this.TestSpan.cloneNode(true);
        test.className +=
          " mjx-test-" + (jax.HTMLCSS.display ? "display" : "inline");
      ...
      for (i = 0; i < m; i++) {
        ...
        jax.HTMLCSS.lineWidth = linebreak
          ? this.length2em(width, 1, maxwidth / this.em)
          : 1000000;
      }
    ...
    Translate: function(script, state) {
      ...
      if (state.HTMLCSSeqn >= state.HTMLCSSlast + state.HTMLCSSchunk) {
        ...
        state.HTMLCSSchunk = Math.floor(
          state.HTMLCSSchunk * this.config.EqnChunkFactor
        );
        ...
        if (script && script.parentNode && script.MathJax.elementJax) {
          ...
          if (jax.HTMLCSS.span && jax.HTMLCSS.div) {
            ...
            if (jax.HTMLCSS.isHidden)
              script.parentNode.insertBefore(jax.HTMLCSS.div, script);
            delete jax.HTMLCSS.span;
            delete jax.HTMLCSS.div;
            //
    ...
    getJaxFromMath: function(math) {
      if (math.parentNode.className.match(/MathJax_Display/)) {
    ...
    Zoom: function(jax, span, math, Mw, Mh) {
      ...
      var bbox = jax.root.HTMLspanElement().bbox,
        width = bbox.width;
      if (width) {
      ...
      if (bbox.H != null && bbox.H > bbox.h) {
        child.marginTop = HTMLCSS.Em(
          bbox.H - Math.max(bbox.h, HTMLCSS.FONTDATA.lineH)
        );
      }
      if (bbox.D != null && bbox.D > bbox.d) {
        child.marginBottom = HTMLCSS.Em(
          bbox.D - Math.max(bbox.d, HTMLCSS.FONTDATA.lineD)
        );
      }
      if (bbox.lw < 0) {
    ...
    getHD: function(span, force) {
      if (span.bbox && this.config.noReflows && !force) {
      ...
      if (span.bbox && this.config.noReflows && span.bbox.exactW !== false) {
        if (!span.bbox.exactW) {
          if (span.style.paddingLeft)
            w += this.unEm(span.style.paddingLeft) * (span.scale || 1);
          if (span.style.paddingRight)
            w += this.unEm(span.style.paddingRight) * (span.scale || 1);
        }
        return w;
      }
    ...
    MeasureSpans: function(SPANS) {
      ...
      for (i = 0, m = spans.length; i < m; i++) {
        span = spans[i];
        if (span[1]) {
          span[1].parentNode.removeChild(span[1]),
            (span[0].style.position = span[3]);
        }
        if (span[2]) {
    ...
    EmRounded: function(m) {
      ...
      m = (Math.round(m * HTMLCSS.em) + 0.05) / HTMLCSS.em;
      return m.toFixed(3).replace(/\.?0+$/, "") + "em";
    },
    unEm: function(m) {
      return parseFloat(m);
    ...
    length2em: function(length, mu, size) {
      ...
      var factor = this.FONTDATA.TeX_factor,
        emFactor = (HTMLCSS.zoomScale || 1) / HTMLCSS.em;
    ...
    getPadding: function(span) {
      ...
      for (var id in padding) {
        if (padding.hasOwnProperty(id)) {
          var pad =
            span.style["padding" + id.charAt(0).toUpperCase() + id.substr(1)];
          if (pad) {
    ...
    getBorders: function(span) {
      ...
      for (var id in border) {
        if (border.hasOwnProperty(id)) {
          ...
          if (style) {
            has = true;
            border[id] = this.border2em(
              span.style[ID + "Width"] || MML.LINETHICKNESS.MEDIUM
            );
            css[ID] = [
              span.style[ID + "Width"],
              span.style[ID + "Style"],
    ...
    createShift: function(span, w, before) {
      var space = this.Element("span", {
        style: { marginLeft: this.Em(w) },
        isMathJax: true
      });
      if (before) {
    ...
    createSpace: function(span, h, d, w, color, isSpace) {
      ...
      var H = this.Em(h + d),
        D = this.Em(-d);
      if (this.msieInlineBlockAlignBug) {
        D = this.Em(HTMLCSS.getHD(span.parentNode, true).d - d);
      }
      if (span.isBox || isSpace) {
        ...
        span = this.addElement(span, "span", {
          style: { height: H, verticalAlign: D },
          isMathJax: true
        });
      }
      if (w >= 0) {
        ...
        if (
          HTMLCSS.safariNegativeSpaceBug &&
          span.parentNode.firstChild == span
        ) {
    ...
    createStack: function(span, nobbox, w) {
      ...
      if (!nobbox) {
        span.parentNode.bbox = span.bbox = {
          ...
          w: W,
          lw: this.BIGDIMEN,
          rw: !relativeW && w != null ? w : -this.BIGDIMEN
        };
        if (relativeW) {
          span.bbox.width = w;
        }
    ...
    createBox: function(span, w) {
      var box = this.addElement(span, "span", {
    ...
    placeBox: function(span, x, y, noclip) {
      span.isMathJax = true;
      var parent = HTMLCSS.parentNode(span),
        bbox = span.bbox,
        BBOX = parent.bbox;
      ...
      if (!span.noAdjust) {
        ...
        if (this.msieInlineBlockAlignBug) {
          ...
          this.addElement(span, "span", {
            isMathJax: true,
            style: { display: "inline-block", width: 0, height: this.Em(HH) }
          });
          if (HTMLCSS.chromeHeightBug) {
            HH -=
              (span.lastChild.offsetHeight - Math.round(HH * this.em)) /
              this.em;
          }
        }
      }
      ...
      if (bbox) {
        if (this.initialSkipBug) {
          ...
          if (bbox.rw > bbox.w) {
            HTMLCSS.createBlank(span, bbox.rw - bbox.w + 0.1);
          }
        }
        if (!this.msieClipRectBug && !bbox.noclip && !noclip) {
          var dd = 3 / this.em;
          var H = bbox.H == null ? bbox.h : bbox.H,
            D = bbox.D == null ? bbox.d : bbox.D;
          var t = HH - H - dd,
            b = HH + D + dd,
            l = -1000,
            r = bbox.rw + 1000;
    ...
    alignBox: function(span, align, y, dx, noskip) {
      ...
      var bbox = span.bbox;
      if (bbox.isMultiline && !noskip) return;
      var isRelative = bbox.width != null && !bbox.isFixed;
      var r = 0,
        c = dx - bbox.w / 2,
        l = "50%";
      if (this.initialSkipBug) {
        r = bbox.w - bbox.rw - 0.1;
        c += bbox.lw;
      }
    ...
    createDelimiter: function(span, code, HW, scale, font) {
      ...
      if (delim.load) {
        HUB.RestartAfter(
          AJAX.Require(this.fontDir + "/fontdata-" + delim.load + ".js")
        );
      }
      for (var i = 0, m = delim.HW.length; i < m; i++) {
        if (
          delim.HW[i][0] * scale >= HW - 0.01 ||
          (i == m - 1 && !delim.stretch)
        ) {
          if (delim.HW[i][2]) {
            scale *= delim.HW[i][2];
    ...
    extendDelimiterH: function(span, W, delim, scale, font) {
      ...
      var w =
          left.bbox.rw - left.bbox.lw + (right.bbox.rw - right.bbox.lw) - 0.05,
        x = left.bbox.rw - left.bbox.lw - 0.025,
        dx;
      if (delim.mid) {
        mid = this.createBox(stack);
        this.createChar(mid, delim.mid, scale, font);
      ...
      if (W > w) {
        var rW = rep.bbox.rw - rep.bbox.lw,
          rw = rW - 0.05,
          n,
          N,
          k = delim.mid ? 2 : 1;
    ...
    createChar: function(span, data, scale, font) {
      ...
      var SPAN = span,
        text = "",
        variant = { fonts: [data[1]], noRemap: true };
    ...
    handleVariant: function(span, variant, text) {
      ...
      if (HTMLCSS.isChar(text) && font.skew && font.skew[n]) {
    ...
    checkFont: function(font, style) {
      ...
      return (
        font.family.replace(/'/g, "") === style.fontFamily.replace(/'/g, "") &&
    ...
    handleFont: function(span, font, force) {
      ...
      if (!font.directory) {
        span.style.fontSize =
          Math.floor(HTMLCSS.config.scale / HTMLCSS.scale + 0.5) + "%";
      }
      if (!(HTMLCSS.FontFaceBug && font.isWebFont)) {
    ...
    handleChar: function(span, font, c, n, text) {
      ...
      if (C.c == null) {
        if (n <= 0xffff) {
          ...
          var N = n - 0x10000;
          C.c =
            String.fromCharCode((N >> 10) + 0xd800) +
      ...
      if (
        c[2] ||
        (!this.msieAccentBug && !this.combiningCharBug) ||
        text.length
      ) {
        return text + C.c;
      }
      if (this.combiningCharBug) {
        HTMLCSS.addElement(
          span,
          "span",
          { style: { marginLeft: HTMLCSS.Em(c[3] / 1000) } },
          [C.c]
        );
        return "";
      }
    ...
    unknownChar: function(variant, n) {
      var unknown = variant.defaultFont || {
        family: HTMLCSS.config.undefinedFamily
      };
      if (variant.bold) {
        unknown.weight = "bold";
      }
  ...
  MathJax.Hub.Register.StartupHook("mml Jax Ready", function() {
    ...
    MML.mbase.Augment(
      {
        ...
        HTMLcomputeBBox: function(span, full, i, m) {
          ...
          var BBOX = (span.bbox = { exactW: true }),
            stretchy = [];
          while (i < m) {
        ...
        HTMLcombineBBoxes: function(core, BBOX) {
          ...
          if (child.style.paddingLeft) {
            BBOX.w +=
              HTMLCSS.unEm(child.style.paddingLeft) * (child.scale || 1);
          }
          ...
          if (child.style.paddingRight) {
            BBOX.w +=
              HTMLCSS.unEm(child.style.paddingRight) * (child.scale || 1);
          }
          if (bbox.width) {
            BBOX.width = bbox.width;
        ...
        HTMLnotEmpty: function(data) {
          while (data) {
            if (
              (data.type !== "mrow" && data.type !== "texatom") ||
              data.data.length > 1
            ) {
              return true;
            }
        ...
        HTMLcreateSpan: function(span) {
          if (this.spanID) {
            ...
            if (
              ...
              (SPAN.parentNode === span ||
                (SPAN.parentNode || {}).parentNode === span)
            ) {
              while (SPAN.firstChild) {
                SPAN.removeChild(SPAN.firstChild);
              }
          ...
          if (this.href) {
            span = HTMLCSS.addElement(span, "a", {
              href: this.href,
              isMathJax: true
            });
          }
          span = HTMLCSS.addElement(span, "span", {
            className: this.type,
            isMathJax: true
          });
          ...
          span.id =
            (this.id || "MathJax-Span-" + this.spanID) + HTMLCSS.idPostfix;
          span.bbox = this.HTMLzeroBBox();
          this.styles = {};
          if (this.style) {
            ...
            this.styles = {
              border: HTMLCSS.getBorders(span),
              padding: HTMLCSS.getPadding(span)
            };
        ...
        HTMLhandleColor: function(span) {
          ...
          if (
            (values.mathbackground &&
              values.mathbackground !== MML.COLOR.TRANSPARENT) ||
            borders ||
            padding
          ) {
            var bbox = span.bbox,
              dd = bbox.exact ? 0 : 1 / HTMLCSS.em,
              lW = 0,
              rW = 0,
              lpad = span.style.paddingLeft,
              rpad = span.style.paddingRight;
            if (this.isToken) {
              lW = bbox.lw;
              rW = bbox.rw - bbox.w;
            }
            ...
            var dw =
              HTMLCSS.PaddingWidthBug || bbox.keepPadding || bbox.exactW
                ? 0
                : rW - lW;
            ...
            var frame = HTMLCSS.Element("span", {
              id: "MathJax-Color-" + this.spanID + HTMLCSS.idPostfix,
              isMathJax: true,
              style: {
                display: "inline-block",
                backgroundColor: values.mathbackground,
                width: HTMLCSS.Em(W),
                height: HTMLCSS.Em(H),
                verticalAlign: HTMLCSS.Em(D),
                marginLeft: HTMLCSS.Em(lW),
                marginRight: HTMLCSS.Em(rW)
              }
            });
        ...
        HTMLremoveColor: function() {
          var color = document.getElementById(
            "MathJax-Color-" + this.spanID + HTMLCSS.idPostfix
          );
          if (color) {
        ...
        HTMLhandleSpace: function(span) {
          if (this.hasMMLspacing()) {
            ...
            if (space !== "") {
              this.HTMLgetScale();
              space =
                (HTMLCSS.length2em(space, this.scale) / (span.scale || 1)) *
                this.mscale;
              if (span.style.paddingLeft) {
        ...
        HTMLgetScale: function() {
          ...
          values.mathsize = (this.isToken ? this : this.Parent()).Get(
            "mathsize"
          );
          if (this.style) {
            var span = this.HTMLspanElement();
        ...
        HTMLgetMu: function(span) {
          ...
          if (values.scriptlevel !== 0) {
            ...
            mu = Math.sqrt(
              Math.pow(values.scriptsizemultiplier, values.scriptlevel)
            );
          }
          return mu;
        },
        ...
        HTMLdrawBBox: function(span) {
          var bbox = span.bbox;
          var box = HTMLCSS.Element(
            "span",
            {
              style: {
                "font-size": span.style.fontSize,
                display: "inline-block",
                opacity: 0.25,
                "margin-left": HTMLCSS.Em(-bbox.w)
              }
            },
            [
              [
                ...
                {
                  style: {
                    height: HTMLCSS.Em(bbox.h),
                    width: HTMLCSS.Em(bbox.w),
              ...
              [
                ...
                {
                  style: {
                    height: HTMLCSS.Em(bbox.d),
                    width: HTMLCSS.Em(bbox.w),
                    "margin-left": HTMLCSS.Em(-bbox.w),
                    "vertical-align": HTMLCSS.Em(-bbox.d),
    ...
    MML.chars.Augment({
      toHTML: function(span, variant, remap, chars) {
        ...
        if (variant.fontInherit) {
          var scale =
            Math.floor(HTMLCSS.config.scale / HTMLCSS.scale + 0.5) + "%";
          HTMLCSS.addElement(span, "span", { style: { "font-size": scale } }, [
            text
          ]);
          if (variant.bold) {
    ...
    MML.mi.Augment({
      toHTML: function(span) {
        ...
        var text = this.data.join(""),
          bbox = span.bbox;
        if (bbox.skew && !HTMLCSS.isChar(text)) {
          delete bbox.skew;
        }
    ...
    MML.mn.Augment({
      ...
      toHTML: function(span) {
        ...
        if (!HTMLCSS.isChar(this.data.join(""))) {
    ...
    MML.mo.Augment({
      toHTML: function(span) {
        ...
        if (values.largeop) {
          variant =
            HTMLCSS.FONTDATA.VARIANT[
              values.displaystyle ? "-largeOp" : "-smallOp"
            ];
        }
        //
        ...
        if (isScript && text.match(/['`"\u00B4\u2032-\u2037\u2057]/)) {
          variant =
            HTMLCSS.FONTDATA.VARIANT["-" + HTMLCSS.fontInUse + "-variant"];
        }
        //
        //  Typeset contents
        //
        ...
        if (values.largeop) {
          ...
          if (
            HTMLCSS.safariVerticalAlignBug &&
            span.lastChild.nodeName === "IMG"
          ) {
            span.lastChild.style.verticalAlign = HTMLCSS.Em(
              HTMLCSS.unEm(span.lastChild.style.verticalAlign || 0) /
                HTMLCSS.em -
                p / span.scale
            );
          } else if (
            HTMLCSS.konquerorVerticalAlignBug &&
            span.lastChild.nodeName === "IMG"
          ) {
            span.style.position = "relative";
      ...
      HTMLcanStretch: function(direction) {
        ...
        this.forceStretch =
          stretch && (this.Get("minsize", true) || this.Get("maxsize", true));
        return stretch;
      },
      HTMLstretchV: function(box, h, d) {
        this.HTMLremoveColor();
        ...
        var span = this.HTMLspanElement(),
          mu = this.HTMLgetMu(span),
          H;
        var scale = this.HTMLgetScale(),
          axis = HTMLCSS.TeX.axis_height * scale;
        if (values.symmetric) {
          H = 2 * Math.max(h - axis, d + axis);
        } else {
          H = h + d;
        }
        values.maxsize = HTMLCSS.length2em(
          values.maxsize,
          mu,
          span.bbox.h + span.bbox.d
        );
        values.minsize = HTMLCSS.length2em(
          values.minsize,
          mu,
          span.bbox.h + span.bbox.d
        );
        ...
        if (H != values.minsize) {
          H = [
            Math.max(
              (H * HTMLCSS.TeX.delimiterfactor) / 1000,
              H - HTMLCSS.TeX.delimitershortfall
            ),
            H
          ];
        }
        span = this.HTMLcreateSpan(box); // clear contents and attributes
        HTMLCSS.createDelimiter(
          span,
    ...
    MML.mtext.Augment({
      toHTML: function(span) {
        ...
        if (
          HTMLCSS.config.mtextFontInherit ||
          this.Parent().type === "merror"
        ) {
          var vname = this.Get("mathvariant");
          if (vname === "monospace") {
            span.className += " MJX-monospace";
          } else if (vname.match(/sans-serif/)) {
    ...
    MML.merror.Augment({
      toHTML: function(span) {
        ...
        //
        var SPAN = MathJax.HTML.addElement(span, "span", {
      ...
      toHTML: function(span, HW, D) {
        ...
        if (this.data[0] != null) {
          ...
          span.bbox = {
            ...
            rw: box.bbox.rw + x,
            H: Math.max(
              box.bbox.H == null ? -HTMLCSS.BIGDIMEN : box.bbox.H + y,
              box.bbox.h + y
            ),
            D: Math.max(
              box.bbox.D == null ? -HTMLCSS.BIGDIMEN : box.bbox.D - y,
              box.bbox.d - y
            )
          };
          if (values.height !== "") {
          ...
          span.bbox.exact = !!(
            ...
            dimen.exec(values.height) ||
            dimen.exec(values.width) ||
            dimen.exec(values.depth)
          );
    ...
    MML.mrow.Augment({
      HTMLlineBreaks: function(span) {
        ...
        return (
          (HTMLCSS.config.linebreaks.automatic &&
            span.bbox.w > HTMLCSS.linebreakWidth) ||
          this.hasNewline()
        );
      },
      HTMLstretchH: function(box, w) {
    ...
    MML.mfrac.Augment({
      toHTML: function(span) {
        ...
        var num = HTMLCSS.createBox(frac),
          den = HTMLCSS.createBox(frac);
        HTMLCSS.MeasureSpans([
          this.HTMLboxChild(0, num),
          this.HTMLboxChild(1, den)
        ]);
        var values = this.getValues(
          "displaystyle",
          "linethickness",
        ...
        if (values.bevelled) {
          ...
          var H =
            Math.max(num.bbox.h + num.bbox.d, den.bbox.h + den.bbox.d) +
            2 * delta;
          ...
          HTMLCSS.placeBox(
            bevel,
            num.bbox.w - delta / 2,
            (bevel.bbox.d - bevel.bbox.h) / 2 + a
          );
          HTMLCSS.placeBox(
            den,
            num.bbox.w + bevel.bbox.w - delta,
            (den.bbox.d - den.bbox.h) / 2 + a - delta
          );
        } else {
          var W = Math.max(num.bbox.w, den.bbox.w);
          var t =
              HTMLCSS.thickness2em(values.linethickness, this.scale) *
              this.mscale,
            p,
            q,
            u,
          ...
          if (t === 0) {
            // \atop
            p = Math.max(
              (isDisplay ? 7 : 3) * HTMLCSS.TeX.rule_thickness,
              2 * mt
            ); // force to at least 2 px
      ...
      HTMLhandleSpace: function(span) {
        if (!this.texWithDelims) {
          ...
          var style =
            span.childNodes[HTMLCSS.msiePaddingWidthBug ? 1 : 0].style;
          style.marginLeft = style.marginRight = HTMLCSS.Em(space);
          span.bbox.w += 2 * space;
          span.bbox.rw += 2 * space;
        }
    ...
    MML.msqrt.Augment({
      toHTML: function(span) {
        ...
        q = Math.max(
          t + p / 4,
          (1.5 * HTMLCSS.TeX.min_rule_thickness) / this.em
        ); // force to be at least 1px
        ...
        if (!ruleC || W < (ruleC.HW[0] || [0])[0] * scale || scale < 0.75) {
    ...
    MML.munderover.Augment({
      toHTML: function(span, HW, D) {
          ...
          if (this.data[i] != null) {
            ...
            if (i == this.base) {
              ...
              if (this.data[this.over] && values.accent) {
                children[i].bbox.h = Math.max(
                  children[i].bbox.h,
                  scale * HTMLCSS.TeX.x_height
                ); // min height of 1ex (#1706)
              }
            } else {
        ...
        var t = HTMLCSS.TeX.rule_thickness * this.mscale,
          factor = HTMLCSS.FONTDATA.TeX_factor;
        var x,
          y,
          z1,
          z2,
        ...
        for (i = 0, m = this.data.length; i < m; i++) {
          if (this.data[i] != null) {
            ...
            if (accent && box.bbox.w <= 1 / HTMLCSS.em + 0.0001) {
              // images can get the width off by 1px
              box.bbox.w = box.bbox.rw - box.bbox.lw;
              box.bbox.noclip = true;
              if (box.bbox.lw) {
                box.insertBefore(
                  HTMLCSS.createSpace(box.parentNode, 0, 0, -box.bbox.lw),
                  box.firstChild
                );
              }
              HTMLCSS.createBlank(box, 0, 0, box.bbox.rw + 0.1);
            }
            dw = {
              left: 0,
              center: (WW - box.bbox.w) / 2,
              right: WW - box.bbox.w
            }[values.align];
            x = dw;
            y = 0;
    ...
    MML.msubsup.Augment({
      toHTML: function(span, HW, D) {
        ...
        var x_height = HTMLCSS.TeX.x_height * scale,
          s = HTMLCSS.TeX.scriptspace * scale * 0.75; // FIXME: .75 can be removed when IC is right?
        var sup, sub;
        ...
        var q = HTMLCSS.TeX.sup_drop * sscale,
          r = HTMLCSS.TeX.sub_drop * sscale;
        var u = base.bbox.h - q,
          v = base.bbox.d + r,
          delta = 0,
          p;
        if (base.bbox.ic) {
          ...
          delta = 1.3 * base.bbox.ic + 0.05; // adjust faked IC to be more in line with expected results
        }
        if (
          this.data[this.base] &&
          HW == null &&
          ...
          if (
            HTMLCSS.isChar(this.data[this.base].data.join("")) &&
            children[0].scale === 1 &&
        ...
        min.subscriptshift =
          min.subscriptshift === ""
            ? 0
            : HTMLCSS.length2em(min.subscriptshift, mu);
        min.superscriptshift =
          min.superscriptshift === ""
            ? 0
            : HTMLCSS.length2em(min.superscriptshift, mu);
        if (!sup) {
          if (sub) {
            v = Math.max(
              v,
              HTMLCSS.TeX.sub1 * scale,
              sub.bbox.h - (4 / 5) * x_height,
              min.subscriptshift
            );
          ...
          if (!sub) {
            ...
            HTMLCSS.placeBox(
              sup,
              base.bbox.w + delta,
              Math.max(u, min.superscriptshift)
            );
            HTMLCSS.placeBox(
              sub,
              base.bbox.w,
              -Math.max(v, min.subscriptshift)
            );
          }
        }
        this.HTMLhandleSpace(span);
    ...
    MML.math.Augment({
      toHTML: function(span, node, phase) {
        ...
        //
        math =
          !phase || phase === HTMLCSS.PHASE.II
            ? HTMLCSS.Measured(html, box)
            : html;
        //
        ...
        if (!phase || phase === HTMLCSS.PHASE.III) {
          ...
          W =
            Math.abs(W) < 0.006
              ? 0
              : Math.max(0, Math.round(W * this.em) + 0.25);
          ...
          if (
            ...
            var shift = HTMLCSS.length2em(
              values.indentshift,
              1,
              HTMLCSS.scale * HTMLCSS.cwidth
            );
            if (this.displayIndent !== "0") {
              var indent = HTMLCSS.length2em(
                this.displayIndent,
                1,
                HTMLCSS.scale * HTMLCSS.cwidth
              );
            ...
            if (shift) {
              ...
              if (color) {
                ...
                color.style.marginRight = HTMLCSS.Em(
                  R +
                    (values.indentalign === "right"
                      ? span.bbox.w + shift - span.bbox.w
                      : 0)
                );
                if (HTMLCSS.msieColorBug && values.indentalign === "right") {
                  if (parseFloat(color.style.marginLeft) > 0) {
                    var padding = MathJax.HTML.addElement(
                      color.parentNode,
                      "span"
                    );
                    padding.style.marginLeft = HTMLCSS.Em(
                      R + Math.min(0, span.bbox.w + shift)
                    );
        ...
        if (this.data[0] != null) {
          if (this.texClass === MML.TEXCLASS.VCENTER) {
            ...
            HTMLCSS.placeBox(
              box,
              0,
              a - (box.bbox.h + box.bbox.d) / 2 + box.bbox.d
            );
          } else {
    ...
    HUB.Browser.Select({
      MSIE: function(browser) {
        ...
        if (mode < 9) {
          ...
          HTMLCSS.config.styles[".MathJax .MathJax_HitBox"][
            "background-color"
          ] = "white";
          HTMLCSS.config.styles[".MathJax .MathJax_HitBox"].opacity = 0;
          HTMLCSS.config.styles[".MathJax .MathJax_HitBox"].filter =
            "alpha(opacity=0)";
        }
      ...
      Opera: function(browser) {
        ...
        HTMLCSS.Augment({
          ...
          PaddingWidthBug: true,
          allowWebFonts:
            browser.versionAtLeast("10.0") && !browser.isMini ? "otf" : false,
