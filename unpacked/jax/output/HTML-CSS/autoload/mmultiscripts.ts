  ...
  MML.mmultiscripts.Augment({
        ...
        while (i < m && (this.data[i] || {}).type !== "mprescripts") {
          ...
          for (var j = k; j < k + 2; j++) {
            if (
              ...
              box[j] = MathJax.HTML.Element("span", {
    ...
    HTMLmoveColor: function(box, w, sign) {
      ...
      if (color && (color.id || "").match(/^MathJax-Color-/)) {
        color.style.marginLeft = HTMLCSS.Em(
          W + parseFloat(color.style.marginLeft)
        );
        color.style.marginRight = HTMLCSS.Em(
          sign * (W - parseFloat(color.style.marginRight))
        );
      }
    },
