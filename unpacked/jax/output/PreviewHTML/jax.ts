  ...
  MathJax.Hub.Register.StartupHook("mml Jax Ready", function() {
    ...
    MML.mbase.Augment({
      ...
      PHTMLdefaultSpan: function(span, options) {
        ...
        for (var i = 0, m = this.data.length; i < m; i++)
          this.PHTMLaddChild(span, i, options);
        return span;
      },
      PHTMLaddChild: function(span, i, options) {
        ...
        if (child) {
          if (options.childSpans)
            span = HTML.addElement(span, "span", {
      ...
      PHTMLcreateSpan: function(span) {
        ...
        return HTML.addElement(span, "span", {
    ...
    MML.mo.Augment({
      ...
      PHTMLstretchV: function(h, d) {
        ...
        var box = HTML.Element("span", {
          style: { "font-size": PHTML.Em(scale) }
        });
        if (scale > 1.25) {
        ...
        span.appendChild(box);
        if (values.symmetric)
          span.style.verticalAlign = PHTML.Em(0.25 * (1 - scale));
      }
    });
    ...
    MML.mpadded.Augment({
      ...
      PHTMLdimen: function(length, d, m) {
        ...
        var size = match
          ? this.PHTML[match[0].charAt(0)]
          : d
          ? this.PHTML[d]
          : 0;
        return {
          len: PHTML.length2em(length, size) || 0,
          pm: !!length.match(/^[-+]/)
        };
      }
    });
    ...
    MML.munderover.Augment({
      toPreviewHTML: function(span) {
        ...
        if (this.data[this.over]) {
          ...
          var over = HTML.Element("span", {}, [
            ["span", { className: "MJXp-over" }]
          ]);
          ...
          this.data[this.over].PHTMLhandleScriptlevel(
            over.firstChild.firstChild
          );
          if (acc != null) {
            if (obox.vec) {
                  ...
                  [
                    ...
                    [
                      [
                        ...
                        {
                          style: {
                            "margin-bottom": PHTML.Em(-(sup.PHTML.d - 0.05))
                          }
                        }
                      ]
                    ]
                  ...
                  [
                    ...
                    [
                      [
                        ...
                        {
                          style: {
                            "margin-top": PHTML.Em(-(sup.PHTML.h - 0.05))
                          }
                        }
                      ]
                    ]
    ...
    MML.mfrac.Augment({
      toPreviewHTML: function(span) {
        ...
        if (!values.displaystyle) {
          if (this.data[0])
            this.data[0].PHTMLhandleScriptlevel(span.firstChild);
        ...
        values.linethickness = Math.max(
          0,
          PHTML.length2em(values.linethickness || "0", 0)
        );
        if (values.linethickness) {
          ...
          rule.style.borderBottom =
            (values.linethickness < 0.15 ? "1px" : t) + " solid";
    ...
    MML.msqrt.Augment({
      ...
      PHTMLlayoutRoot: function(span, base) {
        ...
        var surd = HTML.Element("span", { className: "MJXp-surd" }, [
          [
            "span",
            { style: { "font-size": scale + "%", "margin-top": t } },
            ["\u221A"]
          ]
        ]);
        ...
        if (scale > 150) {
          ...
          surd.firstChild.style.marginLeft = PHTML.Em(
            ((W * (sX / 10 - 1)) / scale) * 100
          );
          W = (W * sX) / 10;
          root.firstChild.style.borderTopWidth = PHTML.Em(
            0.08 / Math.sqrt(sX / 10)
          );
        }
    ...
    MML.mroot.Augment({
      toPreviewHTML: function(span) {
        ...
        if (-r > rbox.w * 0.8)
          root.style.marginLeft = PHTML.Em(-r - rbox.w * 0.8); // ### depends on rbox.w
        span.appendChild(root);
