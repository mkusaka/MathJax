  ...
  nMML.Augment({
    ...
    Config: function() {
      ...
      if (HUB.config.displayAlign !== "center") {
        ...
        HUB.Insert(this.config.styles, {
          ...
          "div.MathJax_MathContainer > span": {
    ...
    Startup: function() {
      ...
      if (!HUB.Browser.hasMathPlayer) {
        ...
        MML.math(MML.mspace().With({ width: "60ex" })).toNativeMML(
    ...
    Translate: function(script) {
      ...
      if (isMSIE) {
        if (container.addEventListener) {
          ...
          var config = (this.config.showMathMenuMSIE != null ? this : HUB)
            .config;
          if (
            config.showMathMenuMSIE &&
    ...
    isFullWidth: function(node) {
      ...
      var width =
        ...
        (String(node.getAttribute("style")).match(/(?:^| )width: *([^; ]*)/) ||
          [])[1];
    ...
    MSIEevent: function() {
      ...
      if (
        event.srcElement.className === "MathJax_MathPlayer_Overlay" &&
  ...
  HUB.Register.StartupHook("mml Jax Ready", function() {
    ...
    MML.mbase.Augment({
      ...
      NativeMMLattributes: function(tag) {
        var defaults =
          this.type === "mstyle" ? MML.math.prototype.defaults : this.defaults;
      ...
      NativeMMLelement: function(type) {
        var math = HUB.Browser.mpNamespace
          ? document.createElement("m:" + type)
    ...
    if (!isMSIE) {
              ...
              if (n) {
                ...
                for (var id in attr) {
                  if (attr.hasOwnProperty(id) && this[id]) {
                    var cols =
                      [attr[id], attr[id]].slice(2 - n).join(" ") + " ";
        ...
        toNativeMML: function(parent) {
          ...
          if (nMML.tableSpacingBug) {
            ...
            var lp = this.parent.nMMLleftPadding,
              M = lp.length;
            for (
              var mtd = mtr.firstChild, i = 0;
              mtd;
              mtd = mtd.nextSibling, i++
            ) {
          ...
          if (nMML.tableLabelBug) {
            ...
            if (this.parent.nMMLhasLabels && mtr.firstChild) {
              ...
              if (forceWidth || side !== "r") {
                ...
                if (align !== "l") {
                  mtr
                    .insertBefore(this.NativeMMLelement("mtd"), mtr.firstChild)
        ...
        toNativeMML: function(parent) {
          ...
          if (nMML.tableLabelBug && this.data[0]) {
            ...
            if (forceWidth || side !== "l") {
              ...
              if (align !== "r") {
                mtr
                  .appendChild(this.NativeMMLelement("mtd"))
      ...
      MML.mspace.Augment({
        toNativeMML: function(parent) {
          ...
          if (nMML.spaceWidthBug && this.width) {
            ...
            var style = (mspace.getAttribute("style") || "").replace(
              /;?\s*/,
              "; "
            );
      ...
      MML.mn.Augment({
        NativeMMLremapMinus: function(text) {
          return text.replace(/^-/, "\u2212");
        },
        toNativeMML: function(parent) {
          var tag = this.NativeMMLelement(this.type);
      ...
      var fontDir = AJAX.fileURL(
        MathJax.OutputJax.fontDir + "/HTML-CSS/TeX/otf"
      );
    ...
    MML.math.Augment({
      toNativeMML: function(parent, jax) {
        ...
        if (mtable.nMMLhasLabels) {
          ...
          if (mtable.nMMLlaMatch) {
            if (parent.parentNode.parentNode.nodeName.toLowerCase() === "div") {
              parent.parentNode.parentNode.style.setProperty(
                "margin-" + HUB.config.displayAlign,
                "0px",
                "important"
              );
            }
        ...
        if (nMML.widthBug && !fullWidth) {
          ...
          //
          parent.style.width =
            (math.firstChild.scrollWidth / nMML.ex / nMML.scale).toFixed(3) +
            "ex";
          //
    ...
    MML.mfenced.Augment({
      toNativeMML: function(parent) {
        ...
        if (values.separators.length == 0) {
          ...
          for (
            i = this.data.length - 1 - values.separators.length;
            i > 0;
            i--
          ) {
