...
(function(AJAX, HUB, HTML, SVG) {
  ...
  SVG.Augment({
    ...
    preTranslate: function(state) {
      ...
      for (i = 0; i < m; i++) {
        ...
        test.className +=
          " mjx-test-" + (jax.SVG.display ? "display" : "inline");
      ...
      for (i = 0; i < m; i++) {
        ...
        jax.SVG.lineWidth = linebreak
          ? this.length2em(width, 1, (maxwidth / em) * 1000)
          : SVG.BIGDIMEN;
      }
    ...
    Translate: function(script, state) {
      ...
      if (this.hideProcessedMath) {
        ...
        if (state.SVGeqn >= state.SVGlast + state.SVGchunk) {
          ...
          state.SVGchunk = Math.floor(
            state.SVGchunk * this.config.EqnChunkFactor
          );
    ...
    postTranslate: function(state, partial) {
      ...
      for (var i = state.SVGlast, m = state.SVGeqn; i < m; i++) {
        ...
        if (script && script.MathJax.elementJax) {
          ...
          //
          script.previousSibling.className = script.previousSibling.className.split(
            / /
          )[0];
    ...
    getJaxFromMath: function(math) {
      if (math.parentNode.className.match(/MathJax_SVG_Display/)) {
    ...
    getPadding: function(styles) {
      ...
      for (var id in padding) {
        if (padding.hasOwnProperty(id)) {
          var pad =
            styles["padding" + id.charAt(0).toUpperCase() + id.substr(1)];
          if (pad) {
        ...
        if (border.hasOwnProperty(id)) {
          ...
          if (style && style !== "none") {
            ...
            border[id] = this.border2em(
              styles[ID + "Width"] || MML.LINETHICKNESS.MEDIUM
            );
    ...
    Element: function(type, def) {
      var obj =
        typeof type === "string" ? document.createElementNS(SVGNS, type) : type;
      obj.isMathJax = true;
      if (def) {
        for (var id in def) {
    ...
    HandleVariant: function(variant, scale, text) {
      ...
      if (
        svg.element.childNodes.length === 1 &&
        !svg.element.firstChild.getAttribute("x")
      ) {
    ...
    createDelimiter: function(code, HW, scale, font) {
      ...
      for (var i = 0, m = delim.HW.length; i < m; i++) {
        if (
          delim.HW[i][0] * scale >= HW - 10 - SVG.config.blacker ||
          (i == m - 1 && !delim.stretch)
        ) {
          if (delim.HW[i][2]) {
            scale *= delim.HW[i][2];
  ...
  var BBOX = (SVG.BBOX = MathJax.Object.Subclass({
    ...
    Align: function(svg, align, dx, dy, shift) {
      dx =
        { left: dx, center: (this.w - svg.w) / 2, right: this.w - svg.w - dx }[
          align
        ] || 0;
      var w = this.w;
  ...
  BBOX.ROW = BBOX.Subclass({
    ...
    Check: function(data) {
      ...
      if (svg.h + svg.y > this.sh) {
        this.sh = svg.h + svg.y;
      }
      if (svg.d - svg.y > this.sd) {
        this.sd = svg.d - svg.y;
      }
    },
    Stretch: function() {
  ...
  BBOX.FRAME = BBOX.Subclass({
    ...
    Init: function(h, d, w, t, dash, color, def) {
      ...
      def.transform =
        "translate(" + Math.floor(t / 2) + "," + Math.floor(-d + t / 2) + ")";
      if (dash === "dashed") {
        def["stroke-dasharray"] = [
          Math.floor(6 * SVG.em),
          Math.floor(6 * SVG.em)
        ].join(" ");
      }
  ...
  BBOX.TEXT = BBOX.Subclass({
    ...
    Init: function(scale, text, def) {
      ...
      this.element.setAttribute(
        "transform",
        "scale(" + SVG.Fixed(scale) + ") matrix(1 0 0 -1 0 0)"
      );
  ...
  HUB.Register.StartupHook("mml Jax Ready", function() {
    ...
    MML.mbase.Augment(
      {
        ...
        toSVG: function() {
          ...
          for (var i = 0, m = this.data.length; i < m; i++) {
            if (this.data[i]) {
              var child = svg.Add(
                this.data[i].toSVG(variant, svg.scale),
                svg.w,
                0,
                true
              );
            ...
            for (var i = 0, m = copy.length; i < m; i++) {
              ...
              if (
                ...
                (!skip[id] &&
                  !ignore[id] &&
                  defaults[id] == null &&
                  typeof svg.element[id] === "undefined")
              ) {
        ...
        SVGprocessStyles: function(style) {
          ...
          if (styles.fontWeight && styles.fontWeight.match(/^\d+$/)) {
            styles.fontWeight =
              parseInt(styles.fontWeight) > 600 ? "bold" : "normal";
          }
          return styles;
        },
        ...
        SVGgetVariant: function() {
          ...
          if (values.family && !values.hasVariant) {
            ...
            variant = {
              forceFamily: true,
              font: { "font-family": values.family }
            };
            if (values.style) {
        ...
        SVGgetScale: function(svg) {
          ...
          if (this.mscale) {
            ...
            if (values.scriptlevel !== 0) {
              ...
              values.scriptminsize =
                SVG.length2em(this.Get("scriptminsize")) / 1000;
    ...
    MML.mo.Augment({
      toSVG: function(HW, D) {
        ...
        if (values.largeop) {
          variant =
            SVG.FONTDATA.VARIANT[values.displaystyle ? "-largeOp" : "-smallOp"];
        }
        //
        ...
        var parent = this.CoreParent(),
          isScript =
            parent && parent.isa(MML.msubsup) && this !== parent.data[0],
      ...
      SVGcanStretch: function(direction) {
        ...
        this.forceStretch =
          can && (this.Get("minsize", true) || this.Get("maxsize", true));
        return can;
      },
      SVGstretchV: function(h, d) {
        ...
        if (H != values.minsize) {
          H = [
            Math.max(
              (H * SVG.TeX.delimiterfactor) / 1000,
              H - SVG.TeX.delimitershortfall
            ),
            H
          ];
        }
        svg = SVG.createDelimiter(
    ...
    MML.mn.Augment({
      ...
      toSVG: function() {
        ...
        for (var i = 0, m = this.data.length; i < m; i++) {
          if (this.data[i]) {
            var child = svg.Add(
              this.data[i].toSVG(variant, svg.scale, remap),
              svg.w,
              0,
              true
            );
    ...
    MML.merror.Augment({
      toSVG: function(HW, D) {
        ...
        this.SVGhandleSpace(svg);
        var def =
          scale !== 1 ? { transform: "scale(" + SVG.Fixed(scale) + ")" } : {};
        var bbox = BBOX(def);
    ...
    MML.mrow.Augment({
      ...
      SVGlineBreaks: function(svg) {
        ...
        return (
          (SVG.config.linebreaks.automatic && svg.w > SVG.linebreakWidth) ||
          this.hasNewline()
        );
      },
      ...
      toSVG: function() {
        ...
        if (values.bevelled) {
          ...
          frac.Add(
            den,
            num.w + bevel.w - delta,
            (den.d - den.h) / 2 + a - delta
          );
        } else {
          var W = Math.max(num.w, den.w);
          var t =
              SVG.thickness2em(values.linethickness, this.scale) * this.mscale,
            p,
            q,
            u,
            v;
    ...
    MML.mroot.Augment({
      ...
      SVGaddRoot: function(svg, surd, x, d, scale) {
        var dx = (surd.isMultiChar ? 0.55 : 0.65) * surd.w;
        if (this.data[1]) {
      ...
      SVGrootHeight: function(d, scale, root) {
        return (
          0.45 * (d - 900 * scale) + 600 * scale + Math.max(0, root.d - 75)
        );
      }
    });
      ...
      toSVG: function(HW, D) {
        ...
        for (i = 0, m = this.data.length; i < m; i++) {
          if (this.data[i] != null) {
            if (i == this.base) {
              ...
              stretch[i] =
                (D != null || HW == null) &&
                this.data[i].SVGcanStretch("Horizontal");
        ...
        for (i = 0, m = this.data.length; i < m; i++) {
          if (this.data[i] != null) {
            ...
            ww = box.w + box.x + (box.X || 0);
            dw = { left: 0, center: (WW - ww) / 2, right: WW - ww }[
              values.align
            ];
            x = dw;
            y = 0;
    ...
    MML.msubsup.Augment({
      toSVG: function(HW, D) {
        ...
        if (base.ic) {
          ...
          delta = 1.3 * base.ic + 0.05; // adjust faked IC to be more in line with expeted results
        }
        if (
          this.data[this.base] &&
          ...
          if (
            SVG.isChar(this.data[this.base].data.join("")) &&
            base.scale === 1 &&
            !base.stretched &&
        ...
        min.subscriptshift =
          min.subscriptshift === "" ? 0 : SVG.length2em(min.subscriptshift, mu);
        min.superscriptshift =
          min.superscriptshift === ""
            ? 0
            : SVG.length2em(min.superscriptshift, mu);
        var x = base.w + base.x;
        if (!sup) {
          if (sub) {
            v = Math.max(
      ...
      toSVG: function(span, div) {
        ...
        if (this.data[0]) {
          ...
          if (Math.abs(w - SVG.cwidth) < 10)
            style.maxWidth =
              SVG.Fixed(((SVG.cwidth * SVG.em) / 1000) * SVG.config.scale) +
              "px";
          //
          ...
          if (alttext && !svg.element.getAttribute("aria-label"))
            svg.element.setAttribute("aria-label", alttext);
          if (!svg.element.getAttribute("role"))
            svg.element.setAttribute("role", "img");
              ...
              HUB.Insert(
                style,
                {
                  ...
                  right: {
                    ...
                    marginLeft: SVG.Ex(Math.max(0, shift - w))
                  },
                  center: {
  ...
  if (!document.createElementNS) {
    ...
    SVG.Augment({
      Element: function(type, def) {
        var obj =
          typeof type === "string"
            ? document.createElement("svg:" + type)
            : type;
        obj.isMathJax = true;
        if (def) {
          for (var id in def) {
