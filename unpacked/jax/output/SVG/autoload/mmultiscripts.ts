  ...
  MML.mmultiscripts.Augment({
    toSVG: function(HW, D) {
      ...
      var base = this.data[this.base]
        ? this.SVGdataStretched(this.base, HW, D)
        : SVG.BBOX.G().Clean();
      var x_height = SVG.TeX.x_height * scale,
        s = SVG.TeX.scriptspace * scale * 0.75; // FIXME: .75 can be removed when IC is right?
    ...
    SVGgetScripts: function(s) {
      ...
      for (var k = 0; k < 4; k += 2) {
        while (i < m && (this.data[i] || {}).type !== "mprescripts") {
          ...
          var isPre = k === 2;
          if (isPre)
            W += Math.max((box[k] || { w: 0 }).w, (box[k + 1] || { w: 0 }).w);
          if (box[k])
            BOX[k].Add(box[k].With({ x: W - (isPre ? box[k].w : 0) }));
          if (box[k + 1])
            BOX[k + 1].Add(
              box[k + 1].With({ x: W - (isPre ? box[k + 1].w : 0) })
            );
          sub = BOX[k] || { w: 0 };
          sup = BOX[k + 1] || { w: 0 };
