...
(function(MML) {
      ...
      SetData: function(i, item) {
        if (item != null) {
          if (!(item instanceof MML.mbase)) {
            item =
              this.isToken || this.isChars ? MML.chars(item) : MML.mtext(item);
          }
          item.parent = this;
  ...
  MML.mi = MML.mbase.Subclass({
    ...
    autoDefault: function(name) {
      if (name === "mathvariant") {
        ...
        return mi.length === 1 ||
          (mi.length === 2 &&
            mi.charCodeAt(0) >= 0xd800 &&
            mi.charCodeAt(0) < 0xdc00)
          ? MML.VARIANT.ITALIC
          : MML.VARIANT.NORMAL;
      }
      return "";
      ...
      for (
        ...
        if (n <= this.RANGES[i][1]) {
          ...
          data = this.OPTABLE.infix[mo] =
            MML.mo.OPTYPES[data === "BIN" ? "BIN3" : data];
    ...
    adjustTeXclass: function(prev) {
      ...
      if (
        ...
        while (
          ...
          parent.isEmbellished() &&
          (parent.data.length === 1 ||
            (parent.type !== "mrow" && parent.Core() === child))
        ) {
          // handles msubsup and munderover
          child = parent;
          parent = parent.parent;
        }
        if (parent.data[parent.data.length - 1] === child)
          this.texClass = MML.TEXCLASS.ORD;
      }
      return this;
    }
  ...
  MML.mspace = MML.mbase.Subclass({
    ...
    hasNewline: function() {
      ...
      return (
        !this.hasDimAttr() && this.Get("linebreak") === MML.LINEBREAK.NEWLINE
      );
    }
  });
      ...
      if (values.separators !== "") {
        ...
        for (var i = 1, m = this.data.length; i < m; i++) {
          if (this.data[i]) {
            this.SetData(
              "sep" + i,
              MML.mo(values.separators.charAt(i - 1)).With({ separator: true })
            );
          }
        }
      }
  ...
  var MO = {
    ...
    OP: [
      ...
      2,
      TEXCLASS.OP,
      { largeop: true, movablelimits: true, symmetric: true }
    ],
