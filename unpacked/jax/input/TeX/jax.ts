...
(function(TEX, HUB, AJAX) {
  ...
  var STARTUP = function() {
    ...
    HUB.Insert(TEXDEF, {
      ...
      mathchar0mi: {
        ...
        backslash: [
          "2216",
          { mathvariant: MML.VARIANT.NORMAL, variantForm: true }
        ],
      ...
      mathchar0mo: {
        ...
        intop: [
          "222B",
          { texClass: MML.TEXCLASS.OP, movesupsub: true, movablelimits: true }
        ],
  ...
  var PARSE = MathJax.Object.Subclass({
    ...
    Limits: function(name, limits) {
      ...
      if (
        !op ||
        (op.Get("texClass") !== MML.TEXCLASS.OP && op.movesupsub == null)
      ) {
        TEX.Error(["MisplacedLimits", "%1 is allowed only on operators", name]);
      }
      var top = this.stack.Top();
      if (op.type === "munderover" && !limits) {
        op = top.data[top.data.length - 1] = MML.msubsup.apply(
          MML.subsup,
          op.data
        );
      } else if (op.type === "msubsup" && limits) {
        op = top.data[top.data.length - 1] = MML.munderover.apply(
          MML.underover,
          op.data
        );
      }
    ...
    Accent: function(name, accent, stretchy) {
      ...
      this.Push(
        MML.TeXAtom(MML.munderover(c, null, mml).With({ accent: true }))
      );
    },
    ...
    TeXAtom: function(name, mclass) {
      ...
      if (mclass == MML.TEXCLASS.OP) {
        ...
        if (match) {
          ...
          mml = STACKITEM.fn(
            MML.TeXAtom(TEX.Parse(arg, this.stack.env).mml()).With(def)
          );
        }
      } else {
    ...
    MmlToken: function(name) {
      ...
      while (attr !== "") {
        ...
        if (
          MML[type].prototype.defaults[match[1]] == null &&
    ...
    InternalMath: function(text, level) {
          ...
          if (c === "$") {
            ...
            if (match === "" && text.substr(i).match(/^(eq)?ref\s*\{/)) {
              ...
              if (c === "(" && match === "") {
                ...
                mml.push(
                  MML.TeXAtom(TEX.Parse(text.slice(k, i - 2), {}).mml())
                );
                match = "";
                k = i;
      ...
      if (level != null) {
        mml = [
          MML.mstyle
            .apply(MML, mml)
            .With({ displaystyle: false, scriptlevel: level })
        ];
  ...
  TEX.Augment({
    ...
    Translate: function(script) {
      ...
      if (mml.isa(MML.mtable) && mml.displaystyle === "inherit")
        mml.displaystyle = display; // for tagged equations
      if (mml.inferred) {
